...
Graph.prototype.removeNode = function(node) {
  for(var key in  this.graph[node].edge){
    delete this.graph[key].edge[node];
  }
...
Graph.prototype.hasEdge = function(fromNode, toNode) {
  if(this.graph[fromNode].edge[toNode] !== undefined){
    return true;
  }else{
    return false;
  }
...
Graph.prototype.addEdge = function(fromNode, toNode) {
  this.graph[fromNode].edge[toNode] = toNode;
  this.graph[toNode].edge[fromNode] = fromNode;
};
